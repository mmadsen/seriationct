...
# TODO - need to do the sampler for lineage splitting
...
def generate_randomized_networkmodel(seed):
    ...
    record['clusterspread'] = nmconfig['clusterspread']

    cmd += " --centroidmin "
    cmd += str(nmconfig['centroidmin'])
    record['centroidmin'] = nmconfig['centroidmin']

    cmd += " --centroidmax "
    cmd += str(nmconfig['centroidmax'])
    record['centroidmax'] = nmconfig['centroidmax']

    cmd += " --nodespercluster "
    cmd += str(nmconfig['nodespercluster'])
    record['nodespercluster'] = nmconfig['nodespercluster']
