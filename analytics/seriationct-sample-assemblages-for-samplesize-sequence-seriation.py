...
def setup():
    ...
    parser.add_argument("--samplefraction", type=float, help="Sample size to resample frequencies for each sim run and replication, governs the largest sample size in the series", required=True)
    parser.add_argument("--smallestsample", type=int, help="Smallest sample size in the series of samples below samplefraction, defaults to 5", default=5)
...
if __name__ == "__main__":
    ...
    for file in os.listdir(args.inputdirectory):
        ...
        if fnmatch.fnmatch(file, '*.txt'):
            ...
            # Get the proper type of sample from the original number of assemblages, with samplefraction assemblages.
            ...
            while sample_size >= args.smallestsample:

                outputfile = args.outputdirectory + "/" + root + "-" + args.sampletype  +"-resample-size-" + str(sample_size) + ".txt"
